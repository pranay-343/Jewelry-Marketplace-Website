      ...
      foreach($categoryList as $data) 
    {
     // print_r($data); die;
      ...
      $c['addedOn'] = date("F j Y g:i a", strtotime($data['added_on']));
       $c['status'] = $status;
      $c['action'] = '<div class="actions"><div class="btn-group"><a class="btn green-haze btn-outline btn-circle btn-sm" href="'.SELLER_URL.'category/?a='.$data['id'].'&b='.$data['status'].'" data-toggle="dropdown" data-hover="dropdown" data-close-others="true"> <i class="fa fa-cogs"></i><i class="fa fa-angle-down"></i></a><ul class="dropdown-menu" style="min-width: 100px !important;"><li><a href="'.SELLER_URL.'category/add-category/?a='.$njEncryptionObj->safe_b64encode($data['id']).'&mode='.  base64_encode('editCategory').'"><i class="fa fa-file-text-o"></i> Edit </a></li><li><a href="javascript:0" onclick="return deleteCategory('.$data['id'].');"><i class="fa fa-file-text-o"></i> Delete </a></li>
