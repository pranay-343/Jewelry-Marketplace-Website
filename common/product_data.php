...
switch ($mode) {
    ...
    break;
     ...
     case 'ProductUserListForm':
        ...
        $table = "products";
        $condition = " 1 and product_type ='simple' and added_by ='".$ss_user_id."' and added_user_type='".$userType."'  and status='1' and `delete`='0'  and product_id <>'$RP_ID' order by product_id desc";
        $result = $dbComObj->viewData($conn, $table, "*", $condition);
        $num = $dbComObj->num_rows($result);
        if ($num > 0) 
        {
